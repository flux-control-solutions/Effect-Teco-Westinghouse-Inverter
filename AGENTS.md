...
- **Safe shutdown** — Registered through `transport.onShutdownForUnits` using the service's own drive set. What a safe state _is_ (a stopped motor) and which units are drives belong to this package; the transport supplies lifecycle ordering and per-unit error isolation. The stop clears the write record first, so a safety action is never suppressed by the cache.
