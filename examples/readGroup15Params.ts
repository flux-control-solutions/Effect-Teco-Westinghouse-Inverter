/**
 * Reads all parameters from Group 15 (PLC Monitoring Parameters) of an A510 inverter.
 *
 * Demonstrates iterating over a parameter group, accessing metadata (name, code),
 * and reading each parameter value concurrently.
 *
 * @example bun run examples/readGroup15Params.ts
 */
...
const program = Effect.gen(function* () {
  ...
  const params = inverter.parameters.group15;

  type EffectValue<F> = F extends Effect.Effect<infer A, any, any> ? A : never;
  type Group15Params = typeof params;
  type Group15Row<K extends keyof Group15Params> = {
    readonly key: K;
    readonly description: Group15Params[K]['meta']['name'];
    readonly value: EffectValue<ReturnType<ReturnType<Group15Params[K]>['read']>>;
  };
  type AnyGroup15Row = {
    [K in keyof Group15Params]: Group15Row<K>;
  }[keyof Group15Params];

  const rows: AnyGroup15Row[] = [];
  ...
  yield* Console.log('=== Group 15: PLC Monitoring Parameters ===');
