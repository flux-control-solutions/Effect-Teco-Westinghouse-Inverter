...
const program = Effect.gen(function* () {
  ...
  const params = inverter.parameters.group09;

  type EffectValue<F> = F extends Effect.Effect<infer A, any, any> ? A : never;
  type Group09Params = typeof params;
  type Group09Row<K extends keyof Group09Params> = {
    readonly key: K;
    readonly description: Group09Params[K]['meta']['name'];
    readonly value: EffectValue<ReturnType<ReturnType<Group09Params[K]>['read']>>;
  };
  type AnyGroup09Row = {
    [K in keyof Group09Params]: Group09Row<K>;
  }[keyof Group09Params];

  const rows: AnyGroup09Row[] = [];
  ...
  yield* Console.log('=== Group 09: Communication Parameters ===');
