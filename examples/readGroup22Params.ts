/**
 * Reads all parameters from Group 22 (PM Motor Parameters) of an A510 inverter.
 *
 * Demonstrates iterating over a parameter group, accessing metadata (name, code),
 * and reading each parameter value concurrently.
 *
 * @example bun run examples/readGroup22Params.ts
 */
...
const program = Effect.gen(function* () {
  ...
  const params = inverter.parameters.group22;

  type EffectValue<F> = F extends Effect.Effect<infer A, any, any> ? A : never;
  type Group22Params = typeof params;
  type Group22Row<K extends keyof Group22Params> = {
    readonly key: K;
    readonly description: Group22Params[K]['meta']['name'];
    readonly value: EffectValue<ReturnType<ReturnType<Group22Params[K]>['read']>>;
  };
  type AnyGroup22Row = {
    [K in keyof Group22Params]: Group22Row<K>;
  }[keyof Group22Params];

  const rows: AnyGroup22Row[] = [];
  ...
  yield* Console.log('=== Group 22: PM Motor Parameters ===');
