/**
 * Reads all parameters from Group 11 (Auxiliary Parameters) of an A510 inverter.
 *
 * Demonstrates iterating over a parameter group, accessing metadata (name, code),
 * and reading each parameter value concurrently.
 *
 * @example bun run examples/readGroup11Params.ts
 */
...
const program = Effect.gen(function* () {
  ...
  const params = inverter.parameters.group11;

  type EffectValue<F> = F extends Effect.Effect<infer A, any, any> ? A : never;
  type Group11Params = typeof params;
  type Group11Row<K extends keyof Group11Params> = {
    readonly key: K;
    readonly description: Group11Params[K]['meta']['name'];
    readonly value: EffectValue<ReturnType<ReturnType<Group11Params[K]>['read']>>;
  };
  type AnyGroup11Row = {
    [K in keyof Group11Params]: Group11Row<K>;
  }[keyof Group11Params];

  const rows: AnyGroup11Row[] = [];
  ...
  yield* Console.log('=== Group 11: Auxiliary Parameters ===');
