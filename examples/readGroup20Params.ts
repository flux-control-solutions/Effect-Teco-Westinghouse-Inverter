/**
 * Reads all parameters from Group 20 (Speed Control Parameters) of an A510 inverter.
 *
 * Demonstrates iterating over a parameter group, accessing metadata (name, code),
 * and reading each parameter value concurrently.
 *
 * @example bun run examples/readGroup20Params.ts
 */
...
const program = Effect.gen(function* () {
  ...
  const params = inverter.parameters.group20;

  type EffectValue<F> = F extends Effect.Effect<infer A, any, any> ? A : never;
  type Group20Params = typeof params;
  type Group20Row<K extends keyof Group20Params> = {
    readonly key: K;
    readonly description: Group20Params[K]['meta']['name'];
    readonly value: EffectValue<ReturnType<ReturnType<Group20Params[K]>['read']>>;
  };
  type AnyGroup20Row = {
    [K in keyof Group20Params]: Group20Row<K>;
  }[keyof Group20Params];

  const rows: AnyGroup20Row[] = [];
  ...
  yield* Console.log('=== Group 20: Speed Control Parameters ===');
