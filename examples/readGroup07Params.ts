...
const program = Effect.gen(function* () {
  ...
  const params = inverter.parameters.group07;

  type EffectValue<F> = F extends Effect.Effect<infer A, any, any> ? A : never;
  type Group07Params = typeof params;
  type Group07Row<K extends keyof Group07Params> = {
    readonly key: K;
    readonly description: Group07Params[K]['meta']['name'];
    readonly value: EffectValue<ReturnType<ReturnType<Group07Params[K]>['read']>>;
  };
  type AnyGroup07Row = {
    [K in keyof Group07Params]: Group07Row<K>;
  }[keyof Group07Params];

  const rows: AnyGroup07Row[] = [];
  ...
  yield* Console.log('=== Group 07: Start/Stop Parameters ===');
