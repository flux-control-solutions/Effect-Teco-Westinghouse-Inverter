...
const program = Effect.gen(function* () {
  ...
  const params = inverter.parameters.group19;

  type EffectValue<F> = F extends Effect.Effect<infer A, any, any> ? A : never;
  type Group19Params = typeof params;
  type Group19Row<K extends keyof Group19Params> = {
    readonly key: K;
    readonly description: Group19Params[K]['meta']['name'];
    readonly value: EffectValue<ReturnType<ReturnType<Group19Params[K]>['read']>>;
  };
  type AnyGroup19Row = {
    [K in keyof Group19Params]: Group19Row<K>;
  }[keyof Group19Params];

  const rows: AnyGroup19Row[] = [];
  ...
  yield* Console.log('=== Group 19: Wobble Frequency Parameters ===');
